...
- [ ] Add a pure matcher for the observed CoreAudio UID format. Compare the full location component, rather than a substring or a display name:
...
- [ ] Add focused unit cases in the same file: `1144400` matches `0x1144400`; `114440` does not; a Loopback UID containing the string does not; a second location `0x1120000` does not select the first device.
- [ ] Run `cargo test -p goxlr-daemon uid_matches_location` and expect all matcher cases to pass.
...
- [ ] Replace the `IOAudioEngine` service matcher with `IOUSBHostDevice`. Read `idVendor`, `idProduct`, and `locationID` only from each USB host device. Accept VID `0x1220` with PID `0x8fe0` or `0x8fe4`.
- [ ] Enumerate HAL device IDs with `kAudioHardwarePropertyDevices` and read each device UID using the existing `get_uid_for_id` helper. Select one UID with `uid_matches_location`; return an error on two matches, and do not select by the name `GoXLR`.
- [ ] Release IOKit iterators and objects on every path. Skip malformed USB entries without panicking. Use the verified PID to set the display name to `GoXLR` or `GoXLR Mini`.
- [ ] Run `cargo fmt --check`, `cargo check -p goxlr-daemon`, and the focused matcher test. Expect success.
...
- [ ] Verify on macOS 27 that IOKit reports `0x1220:0x8fe0` and a location ID matching exactly one live CoreAudio UID. Record only the non-sensitive identifiers.
- [ ] Preserve the current `macos_handle_aggregates = false` setting and Loopback configuration. Use a temporary, explicitly named aggregate test device that is destroyed after inspection.
